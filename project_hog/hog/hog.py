...
def free_bacon(score):
    ...
    def abs( x, y):
        if x-y> 0:
            ...
            return y-x
    return 2+ abs(tens_unit, ones_unit)
    #2+ abs(score//10 - score%10)
...
def is_swap(score0, score1):
    ...
    if (score0 != 0 and score0 !=1) and (score1 != 0 and score1 !=1):
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def sub_func(score0, score1):
        ...
        if points()> previous_high:
            prev_high = points()
            if points() == 1:
                print(f"1 point! That's the biggest gain yet for Player {who}")
                return announce_highest(who, prev_high, prev_score)
            else: 
                print(f"{points()} points! That's the biggest gain yet for Player {who}")
                return announce_highest(who, prev_high, prev_score)
        else:
            return announce_highest(who, previous_high, prev_score)
    return sub_func
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for rolls in range(1,11):
        ...
        lists.append(average(rolls, dice ))
    maximum = max(lists)
    for roll in range(1,11):
        average = make_averaged(roll_dice,num_samples )
        if average(roll, dice) == maximum:
            largest_ones.append(roll)
    return min(largest_ones)
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 11
    if opponent_score > free+score and is_swap(free+score, opponent_score):
        return 0
    
    if not bacon_strategy(score, opponent_score, margin, num_rolls):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
