...
def flatten(lst):
    ...
    for i in lst:
        if type(i) == list:
            for x in i:
                lst.insert(lst.index(i), x )
                #pushes the list back then takes its place
            lst.remove(i)
            return flatten(lst)
    return lst
...
def merge(lst1, lst2):
    ...
    #         if x != len(lst1) -1:
    #             if lst1[x] < i < lst1[x+1]:
    #                 lst1.insert(x+1,i)
    #                 # removing 'i' interferes with the initial for loop
    # for i in lst2:
    #     lst1.append(i)
...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(max_rows-1, -1,-1):
        if get_piece(board, i, column ) =='-':
            lst.append(i)
            lst.append(replace_elem(board, i , replace_elem(board[i], column, player)))
            break
        elif i == 0:
            lst.append(-1)
            lst.append(board)
...
def print_board(board, max_rows, max_cols):
    ...
    """
    "*** YOUR CODE HERE ***"
    def recall(num, col , count):
        if count == col-1:
            return str(board[num][count]) + recall(num, col, count+1) 
        elif count < col :
            return str(board[num][count])+ " " + recall(num, col, count+1)
        else:
            return '' 
    for i in range(0, max_rows):
        print(recall(i, max_cols, 0))
