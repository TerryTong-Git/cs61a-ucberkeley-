...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if insect.name == "Queen" and insect.true_queen == "me":
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if self.place.ant:
            return self.place.ant.blocked_path 
        else:
            return False
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while check_place != furthest_place and not check_place.bees: #the furthest place is inclusive if it is NOT the hive, but exclusive if it is. Make the next segment reflect this. 
            check_place = check_place.entrance
        if check_place ==hive:
            return
        else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            self.place.ant = None
            bees_alive = copy.copy(self.place.bees)
            for bee in self.place.bees:
                bee.armor -= self.damage
                if bee.armor <= 0:
                    bees_alive.remove(bee)
                else:
                    bees_alive[bees_alive.index(bee)] = bee
            self.place.bees = bees_alive
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting != 0 and self.stomach: # if digesting and stomach is full
            self.digesting -=1
        else:
            if self.stomach:
                self.stomach = []
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if self.true_queen == None:
            self.true_queen = "me"
            QueenAnt.true_queen = "exists"
        else:
            self.true_queen = "imposter"
    ...
    def action(self, colony):
        ...
        if self.true_queen == "imposter":
            ...
            #     if place.ant:
            #         if place.ant.container and place.ant not in self.buffed_ants:
            #             place.ant.damage = place.ant.damage * 2
            #             self.buffed_ants.append(place.ant)
            #             if place.ant.ant and place.ant.ant not in self.buffed_ants:
            #                 place.ant.ant.damage = place.ant.ant.damage * 2
            #                 self.buffed_ants.append(place.ant.ant)
            #         elif place.ant not in self.buffed_ants:
            #             place.ant.damage = place.ant.damage*2
            #             self.buffed_ants.append(place.ant)
            #     place = place.exit
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor ==0 and self.true_queen == "me":
            bees_win()
